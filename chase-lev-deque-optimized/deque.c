...
int take(Deque *q) {
	...
	if (t <= b) {
		...
		if (t == b) {
			...
			if (!atomic_compare_exchange_strong_explicit(&q->top, &t, t + 1, memory_order_acq_rel, memory_order_acquire))
				/* Failed race. */
				x = EMPTY;
...
int steal(Deque *q) {
	...
	if (t < b) {
		...
		if (!atomic_compare_exchange_strong_explicit(&q->top, &t, t + 1, memory_order_release, memory_order_relaxed))
			/* Failed race. */
			return ABORT;
	}
	return x;
